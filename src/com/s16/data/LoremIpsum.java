...
public class LoremIpsum {
	...
	protected static final String TAG = LoremIpsum.class.getSimpleName();
	...
	private boolean mStartWithLoremIpsum = true;
	...
	*/
	public String[] getParagraphs(int numberOfParagraphs) {
		return getLoremIpsum(1, numberOfParagraphs, mStartWithLoremIpsum);
	}
	...
	*/
	public String[] getWords(int numberOfWords) {
		return getLoremIpsum(3, numberOfWords, mStartWithLoremIpsum);
	}
	...
	*/
	public void setStartWithLoremIpsum(boolean startWithLoremIpsum) {
		this.mStartWithLoremIpsum = startWithLoremIpsum;
	}
	...
	private String[] getLoremIpsum(int type, int count, boolean start) {
		List<String> loremIpsumArray = new ArrayList<String>();
		
		switch(type) {
			case 1: // paragraphs
				for (int i = 0; i < count; i++) {
					int paragraphLength = randomInt(10, 20);
					String[] sentences = getLoremIpsum(2, paragraphLength, (start && i == 0));
			...
			case 2: // sentences
				for (int i = 0; i < count; i++) {
					int sentencesLength = randomInt(5, 10);
					String loremIpsumString = getLoremIpsumString(sentencesLength, (start && i == 0));
					if (loremIpsumString.charAt(loremIpsumString.length() - 1) == ',') {
						loremIpsumString = loremIpsumString.substring(0, loremIpsumString.length() - 1);
					}
					loremIpsumString = loremIpsumString.substring(0, 1).toUpperCase() + loremIpsumString.substring(1) + ".";
					loremIpsumArray.add(loremIpsumString);
				}
				break;
				
			case 3: // words
				loremIpsumArray.add(getLoremIpsumString(count, start));
				break;
		...
		String[] paragraphs = new String[loremIpsumArray.size()];
	    paragraphs = loremIpsumArray.toArray(paragraphs);
		return paragraphs;
	}
	
	private String getLoremIpsumString(int count, boolean start) {
		if (start) {
			String[] tokens = Arrays.copyOfRange(LOREM_WORDS, 0, count);
			return TextUtils.join(" ", tokens);
		} else {
			int wordIndex = randomInt(0, LOREM_WORDS.length - count - 1);
			String[] tokens = Arrays.copyOfRange(LOREM_WORDS, wordIndex, wordIndex + count);
			return TextUtils.join(" ", tokens); //.replaceAll("\\.|\\,", "");
		}
	}
}
