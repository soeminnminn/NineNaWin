...
public class SlidingUpTabDrawer extends ViewGroup {
     ...
     */
    public SlidingUpTabDrawer(Context context, AttributeSet attrs) {
     ...
     */
    public SlidingUpTabDrawer(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        
        final DisplayMetrics dm = getResources().getDisplayMetrics();
        mTapThreshold = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, TAP_THRESHOLD, dm);
        mMaximumTapVelocity = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, MAXIMUM_TAP_VELOCITY, dm);
        mMaximumMinorVelocity = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, MAXIMUM_MINOR_VELOCITY, dm);
        mMaximumMajorVelocity = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, MAXIMUM_MAJOR_VELOCITY, dm);
        mMaximumAcceleration = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, MAXIMUM_ACCELERATION, dm);
        mVelocityUnits = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, VELOCITY_UNITS, dm);
        /*		
		...
		*/
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SlidingUpTabDrawer, defStyle, 0);

        mVertical = true;
        mBottomOffset = (int) a.getDimension(R.styleable.SlidingUpTabDrawer_sudBottomOffset, 0.0f);
        mTopOffset = (int) a.getDimension(R.styleable.SlidingUpTabDrawer_sudTopOffset, 0.0f);
        mAllowSingleTap = true;
        mAnimateOnClick = a.getBoolean(R.styleable.SlidingUpTabDrawer_sudAnimateOnClick, true);
        mRightTab = a.getBoolean(R.styleable.SlidingUpTabDrawer_sudRightTab, true);
        mBackgroundColor = a.getColor(R.styleable.SlidingUpTabDrawer_sudBackgroundColor, 0);
        
        int handleId = a.getResourceId(R.styleable.SlidingUpTabDrawer_sudHandle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.SlidingUpTabDrawer_sudContent, 0);
        if (contentId == 0) {
    ...
    public void setBottomOffset(int mBottomOffset) {
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(SlidingUpTabDrawer.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(SlidingUpTabDrawer.class.getName());
    }
	...
	private static class SlidingHandler extends Handler {
		private final WeakReference<SlidingUpTabDrawer> mService;

		SlidingHandler(SlidingUpTabDrawer service) {
			mService = new WeakReference<SlidingUpTabDrawer>(service);

		}

		public void handleMessage(Message m) {
			SlidingUpTabDrawer service = mService.get();
			if (service != null) {
				switch (m.what) {
				case MSG_ANIMATE:
					service.doAnimation();
